...
import { Search, FolderPlus, Folder, FileText, FileSpreadsheet, Presentation, FileType2, RefreshCw, ExternalLink, MapPin, Trash2, CircleAlert, X, Keyboard, PanelRightClose, PanelRightOpen, CheckCircle2, LoaderCircle, Code2, Copy, CircleHelp, Radio, ChevronDown, Image as ImageIcon, Music, Video } from 'lucide-react'

const types = [
  { key: 'all', label: '全部' }, { key: 'office', label: '办公文档' }, { key: 'wps', label: 'WPS' },
  { key: 'code', label: '代码文件' }, { key: 'text', label: '文本文件' }, { key: 'media', label: '媒体文件' }
]
const typeMeta = {
  ...
  code: { label: '<>', color: '#6e56cf', Icon: Code2 }, image: { label: 'IMG', color: '#0e7490', Icon: ImageIcon },
  audio: { label: 'AUD', color: '#7c3aed', Icon: Music }, video: { label: 'VID', color: '#db3c30', Icon: Video }
}
...
function MediaPreview({ result }) {
  ...
  if (result.type === 'audio') return <div className="media-preview audio-preview"><Music size={34}/><strong>{result.name}</strong><audio controls preload="metadata" src={url}>当前系统无法播放此音频格式。</audio></div>
  if (result.type === 'video') return <div className="media-preview video-preview"><video controls preload="metadata" src={url}>当前系统无法播放此视频格式。</video></div>
  return null
}
...
function DetailPane({ result, onClose, onOpen, onReveal, onCopy, opening }) {
  ...
  const isMedia = result.type === 'image' || result.type === 'audio' || result.type === 'video'
  const previewLabel = result.type === 'image' ? '图片预览' : result.type === 'audio' ? '音频播放' : result.type === 'video' ? '视频播放' : '内容预览'
...
function IndexProgressDialog({ progress, onBackground }) {
  ...
  return <div className="modal-backdrop" role="presentation">
    <section className="index-dialog" role="dialog" aria-modal="true" aria-labelledby="index-dialog-title">
      ...
      <div className="index-dialog-copy"><h2 id="index-dialog-title">{scanning ? '正在扫描文件夹' : '正在建立内容索引'}</h2><p>{scanning ? '正在查找办公文档、WPS、代码、文本和媒体文件…' : `已处理 ${progress.done} / ${progress.total} 个文件`}</p></div>
...
function SyntaxHelp() {
  return <details className="syntax-help"><summary><CircleHelp size={14}/>搜索语法<ChevronDown size={13}/></summary><div className="syntax-card"><strong>组合搜索示例</strong><code>"设备检修" -作废</code><code>name:报告 ext:et</code><code>type:image / audio / video</code><p>双引号表示精确短语，减号排除关键词；支持 name、content、ext、type。</p></div></details>
}
